...
import { adminReadLimiter } from '../../middleware/rateLimit.js';

/**
 * `/api/v1/admin/overview` (docs/architecture/03 §5, docs/architecture/07
 * §3). `authenticate` before `adminReadLimiter` here, not the public-route
 ...
 * rateLimit.ts`'s `adminReadLimiter`), which needs `req.user` populated
 * first.
 *
...
overviewRouter.get('/', authenticate, adminReadLimiter, overviewController.show);
