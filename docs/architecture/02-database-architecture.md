...
**Engine:** SQLite (WAL) · **ORM:** Prisma 7 · **Migrations:** `prisma migrate` (+ hand-written raw
SQL for `CHECK` constraints, a partial index, and FTS5).

> **Correction from Phase 2.** Prisma 7 removed the bundled Rust query engine for most providers —
> `PrismaClient` now **requires** a driver adapter constructor argument (`adapter:
> runtime.SqlDriverAdapterFactory`); this is not an opt-in preview feature, and there is no other way
> to construct the client for SQLite. This project uses `@prisma/adapter-better-sqlite3`. It changes
> nothing about the schema or the migration SQL described below, but it does change
> `prisma.config.ts` (a new file the CLI needs, separate from the schema) and `src/config/prisma.ts`
> (the app's own client singleton) — see that file for the exact construction and where the startup
> PRAGMAs in §1 below are actually applied. Verified empirically against prisma@7.10.0 before writing
> any of this, rather than assumed from older Prisma versions' behaviour.
...
- `CHECK` constraints, partial indexes, FTS5 tables and triggers live in hand-authored SQL, worked
  into the migration generated by `prisma migrate dev --create-only`.
  **Correction from Phase 2** (verified against the real engine — SQLite rejects the naive reading
  of "appended"): a `CHECK` constraint cannot be added with `ALTER TABLE` after the fact — SQLite
  only accepts it as part of the column definition at `CREATE TABLE` time. So `CHECK`s are inlined
  into the generated `CREATE TABLE` statements themselves; only the partial index, the FTS5 virtual
  table and its triggers are genuinely *appended* after the generated SQL. The workflow: generate
  with `--create-only`, hand-edit the file, apply with `prisma migrate deploy` (`migrate dev` without
  `--create-only` prompts interactively even when nothing is pending — always use `deploy` to apply
  an already-written migration file, in scripts and in CI alike).
